...
set.seed(1)
mod_fit <- mod_spec |>
  fit(Surv(time, status == 2) ~ sex, dados)
mod_fit
```
...
```{r}
rec <- recipe(time + status ~ sex, lung) |>
  step_mutate(sex = factor(sex)) |>
  step_mutate(outcome = Surv(time, status == 2), role = "outcome", skip = TRUE) |>
  step_rm(time, status, skip = TRUE)
...
mod_fit |>
  predict(new_data |> mutate(sex = as.character(sex)), type = "survival", time = 100) |>
  tidyr::unnest(cols = .pred)

mod_tidy |>
  predict(new_data, type = "survival", time = 100) |>
  tidyr::unnest(cols = .pred)
```