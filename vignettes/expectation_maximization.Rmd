...
dir.create(file.path('data/', 'expectation_maximization'), showWarnings = FALSE)

if(!file.exists('data/expectation_maximization/data.rds')) {
  data <- simulate_data(6000,
    mixture_components = 3, k = 2,
    percentage_censored = 0.3
  )$data |>
  ...
  write_rds(data, 'data/expectation_maximization/data.rds')
}

data <- read_rds("data/expectation_maximization/data.rds")

if(!file.exists('data/expectation_maximization/model_em1.rds')){
  model_em <- survival_ln_mixture_em(Surv(y, delta) ~ x,
    data = data,
    iter = 200,
    starting_seed = 20,
  ...
  write_rds(model_em, 'data/expectation_maximization/model_em1.rds')
}

model_em <- read_rds('data/expectation_maximization/model_em1.rds')

if(!file.exists('data/expectation_maximization/gg1.rds')){
  write_rds(plot_fit_on_data_em(model_em), 'data/expectation_maximization/gg1.rds')
}

gg <- read_rds('data/expectation_maximization/gg1.rds')
```
...
When using the Expectation-Maximization algorithm to fit the model, you can use the function `plot_fit_on_data_em()` to quickly visualize the model's estimated survival (or hazard) on the data used to fit the model.

```{r}
plot_fit_on_data(model_em, type = 'survival')
```
...
if(!file.exists('data/expectation_maximization/gg2.rds')) {
  write_rds(plot_fit_on_data_em(model_em), 'data/expectation_maximization/gg2.rds')
}

gg <- read_rds('data/expectation_maximization/gg2.rds')
```

Now, we have a maximum likelihood estimator that tries to avoid local maximas. As before, we can use the function `plot_fit_on_data_em()` to visualize the model's estimated survival on the data used to fit the model.

```{r}
plot_fit_on_data_em(model_em, type = 'survival')
```
