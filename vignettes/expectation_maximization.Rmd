...
```{r}
new_data <- dplyr::distinct(data, x)
predictions <- predict(model_em,
                       new_data,
                       type = 'survival',
                       eval_time = 1:300) |> 
  tidyr::unnest(.pred)
...
ggplot() +
  geom_path(aes(x = time, y = estimate, color = strata), data = km_surv,
            alpha = 0.6) +
  geom_line(aes(x = .eval_time, y = .pred_survival, color = factor(x1)), data = predictions) +
  theme_light()
```
