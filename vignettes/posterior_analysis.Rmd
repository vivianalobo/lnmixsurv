...
start <- Sys.time()
set.seed(3)
mod_sequential <- survival_ln_mixture(Surv(y, delta) ~ x, data = sim_data$data, chains = 6)
# Time to run 6 chains sequentially:
print(Sys.time() - start)

start <- Sys.time()
set.seed(3)
mod_parallel <- survival_ln_mixture(Surv(y, delta) ~ x, data = sim_data$data, chains = 6,
                    cores = 6, parallel = TRUE)
