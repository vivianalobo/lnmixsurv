...
dados <- lung %>%
  mutate(x = ifelse(sex == 1, "M", "F"), delta = status - 1) %>%
  rename(y = time)
...
surv_km <- tidy_survfit(km, type = "surv") %>%
  select(.time = time, .pred_survival = estimate, id = strata) %>%
  tidyr::nest(.pred = c(.time, .pred_survival))
```
...
ggplot(aes(x = .time, y = .pred_survival, col = id), data = all_preds) +
  theme_bw() +
  geom_line() +
  facet_wrap(~modelo) +
