#' Lognormal mixture model - Stochastic EM algorithm for censored data
#'
#' `survival_ln_mixture_em()` fits a lognormal mixture model using the SEM algorithm for censored data, a modification of the EM described in LOBO, Viviana GR; FONSECA, Thaís CO; ALVES, Mariane B. Lapse risk modeling in insurance: a Bayesian mixture approach. Annals of Actuarial Science, v. 18, n. 1, p. 126-151, 2024.
#'
...
#'
#' @export
survival_ln_mixture_sem <- function(y, delta, X, iter = 50, mixture_components = 2,
                                    starting_seed = sample(1:2^28, 1)) {
  matrix_em_iter <- lognormal_mixture_sem(iter, mixture_components, y, delta, X,
                                          starting_seed)
